...
@interface NSString (llvmIntrospection)

+ (NSString *)decodeType:(const char *)cString;

@end

@implementation NSString (llvmIntrospection)
    ...
    for (unsigned int i = 0; i < outCount; i++) {
        ...
        for (int idx = offset; idx < args; idx++) {
            ...
            selParts[idx - offset] = [NSString stringWithFormat:@"%@:(%@)arg%d",
                                      ...
                                      returnType,
                                      idx - 2];
            NSLog(@"---->>>>>>>4：returnType:%@",returnType);
        }
        NSLog(@"---->>>>>>>5：selParts:%@",[selParts componentsJoinedByString:@" "]);
